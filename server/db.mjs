...
export function seed(db) {
  ...
  for (const [id, name, type] of [['stripe', 'Stripe', 'stripe'], ['xero', 'Xero Payslips', 'xero'], ['bank', 'Bank Transactions', 'bank'], ['manual', 'Manual Transactions', 'manual']]) {
    db.prepare('INSERT OR IGNORE INTO accounts VALUES (?, ?, ?, 1, ?)').run(id, name, type, now);
  }
  for (const [id, name, kind, business] of [
    ['income-stripe', 'Stripe Income', 'income', 1],
    ['income-affiliate', 'Affiliate Marketing Income', 'income', 1],
    ['income-xero', 'Employment Income', 'income', 0],
    ['software', 'Software', 'expense', 1],
    ['travel', 'Travel', 'expense', 1],
    ['education', 'Education', 'expense', 1],
    ['personal', 'Personal', 'expense', 0],
    ['uncategorized', 'Uncategorised', 'expense', 0],
  ]) db.prepare('INSERT OR IGNORE INTO categories VALUES (?, ?, ?, ?, ?)').run(id, name, kind, business, now);
...
function migrate(db) {
  db.exec(`
    ...
    CREATE TABLE IF NOT EXISTS holdings (id TEXT PRIMARY KEY, name TEXT NOT NULL, units REAL, value_minor INTEGER NOT NULL, as_of_date TEXT NOT NULL, created_at TEXT NOT NULL, category TEXT NOT NULL DEFAULT 'etf', monthly_contribution_minor INTEGER NOT NULL DEFAULT 0, return_mtd_minor INTEGER NOT NULL DEFAULT 0, annual_return_minor INTEGER NOT NULL DEFAULT 0, updated_at TEXT);
  ...
  addColumn(db, 'import_jobs', 'period_start TEXT');
  addColumn(db, 'import_jobs', 'period_end TEXT');
  addColumn(db, 'import_jobs', 'summary_json TEXT');
  addColumn(db, 'import_jobs', 'reverted_at TEXT');
  addColumn(db, 'assets', "category TEXT NOT NULL DEFAULT 'savings'");
  addColumn(db, 'liabilities', "category TEXT NOT NULL DEFAULT 'credit_card'");
  addColumn(db, 'holdings', "category TEXT NOT NULL DEFAULT 'etf'");
  addColumn(db, 'holdings', 'monthly_contribution_minor INTEGER NOT NULL DEFAULT 0');
  addColumn(db, 'holdings', 'return_mtd_minor INTEGER NOT NULL DEFAULT 0');
  addColumn(db, 'holdings', 'annual_return_minor INTEGER NOT NULL DEFAULT 0');
  addColumn(db, 'holdings', 'updated_at TEXT');
  addColumn(db, 'transactions', "reconciliation_status TEXT NOT NULL DEFAULT 'unmatched'");
  addColumn(db, 'transactions', 'reconciled_transaction_id TEXT');
  seed(db);
}

export function id(prefix) { return `${prefix}_${crypto.randomUUID()}`; }
export function audit(db, entityType, entityId, action, before, after) {
  db.prepare('INSERT INTO audit_events VALUES (?, ?, ?, ?, ?, ?, ?)').run(id('audit'), entityType, entityId, action, before ? JSON.stringify(before) : null, after ? JSON.stringify(after) : null, new Date().toISOString());
}
